...
public class PersonMapper {
    ...
    public static Person updateEntity(Person person, PersonUpdateRequestDTO personRequest) {
        ...
        if (personRequest.getUserId() != null) {
            // If userId is provided, associate with that user
            person.setUser(User.builder().id(personRequest.getUserId()).build());
        } else if (personRequest.getUserId() == null && person.getUser() != null) {
            // If userId is explicitly null and person has a user, disassociate
            person.setUser(null);
        }
        // If userId is not provided in the request, keep existing association
