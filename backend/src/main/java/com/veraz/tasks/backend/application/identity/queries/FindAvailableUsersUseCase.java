...
public class FindAvailableUsersUseCase {
    
    private final UserRepository userRepository;
    private final UserResponseMapper userResponseMapper;
    
    public FindAvailableUsersUseCase(UserRepository userRepository, UserResponseMapper userResponseMapper) {
        this.userRepository = userRepository;
        this.userResponseMapper = userResponseMapper;
    }
    
    public PaginatedResponseDTO<UserResponse> execute(PaginationRequestDTO paginationRequest) {
        Page<User> usersPage = userRepository.findAvailableUsers(paginationRequest);
